...
foreach ($test in $testMessages) {
    ...
    try {
        $body = @{ message = $test.message } | ConvertTo-Json -Compress
        $utf8Body = [System.Text.Encoding]::UTF8.GetBytes($body)
        
        $response = Invoke-WebRequest -Uri "$baseUrl/check/message" -Method POST -Body $utf8Body -ContentType "application/json; charset=utf-8" -UseBasicParsing -TimeoutSec 30
