...
class AIEngine:
    ...
    async def analyze_text(self, text: str) -> Dict:
        ...
        prediction = "High Risk" if confidence > 0.5 else "Low Risk"
        
        result = {
            "prediction": prediction,
            "confidence": confidence if prediction == "High Risk" else float(proba[0]),
            "is_suspicious": prediction == "High Risk",
            "explanation_en": "Basic pattern matching applied.",
            "explanation_bn": "প্রাথমিক প্যাটার্ন ম্যাচিং প্রয়োগ করা হয়েছে।",
            "red_flags": []
        }
        
        # 2. LangChain LLM Reasoning (if configured)
        if self.openai_client:
            try:
                from app.services.ai_service import get_ai_service
                ai_service = get_ai_service()
                llm_result = await ai_service.analyze_message(text)
                if llm_result:
                    result.update({
                        "explanation_en": llm_result.get("explanation_en", result["explanation_en"]),
                        "explanation_bn": llm_result.get("explanation_bn", result["explanation_bn"]),
                        "red_flags": llm_result.get("red_flags", []),
                        "risk_level": llm_result.get("risk_level", "Medium"),
                        "ai_confidence": llm_result.get("confidence_score", confidence)
                    })
            except Exception as e:
                print(f"LangChain LLM Reasoning failed: {e}")
